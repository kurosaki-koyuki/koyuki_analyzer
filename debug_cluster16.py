# -*- coding: utf-8 -*-
"""
调试脚本16：查看 clusterTrackingPlot 并测试完整流程
"""
import sys
import os
import tempfile
...
print("=== 查看 clusterTrackingPlot 函数 ===")
r_code0 = '''
library(ConsensusClusterPlus)
clusterTrackingPlot <- ConsensusClusterPlus:::clusterTrackingPlot
print(clusterTrackingPlot)
'''
try:
    robjects.r(r_code0)
...
print("\n=== 测试完整 ConsensusClusterPlus 调用，但先关闭自动绘图 ===")
r_code = '''
...
tryCatch({
  results <- ConsensusClusterPlus(exprSet,
                                  ...
                                  seed = 123456,
                                  plot = NULL)
  print("ConsensusClusterPlus 成功！")
  print(paste("结果长度:", length(results)))
}, error = function(e) {
  cat("\\n========== 错误 ==========\\n")
  print(e$message)
  cat("\\n========== 完整调用栈 ==========\\n")
