...
class BulkDiffPyPageUI:
    ...
    def update_styles(self):
        ...
        title_label = self.bulk_diff_page.findChild(QLabel, "bulk_diff_py_title")
        if title_label:
        ...
        for child in self.bulk_diff_page.findChildren(QLabel):
            if child.objectName() != "bulk_diff_py_title":
    ...
    def create_page(self):
        ...
        title_label = QLabel("Python版本差异分析")
        title_label.setObjectName("bulk_diff_py_title")
        title_label.setFont(get_font_for_widget('button', 32, bold=True))
        title_label.setStyleSheet(f"color: {styles.get('sub_mutant_color', '#FF6B35')};")
        ...
        self.diff_method_combo.addItems(["t检验", "Mann-Whitney U检验"])
        self.diff_method_combo.setCurrentIndex(0)
        self.diff_method_help_btn = create_questions_button(
            "t检验适用于正态分布数据，Mann-Whitney U检验为非参数检验，适用于非正态分布数据。\n"
            "分析前会自动进行标准化处理。"
        )
        ...
        self.diff_result_table.setHorizontalHeaderLabels([
            "基因", "mean_group1", "mean_group2", "log2FC",
            "p_val", "p_val_adj", "significant", "direction"
        ])
