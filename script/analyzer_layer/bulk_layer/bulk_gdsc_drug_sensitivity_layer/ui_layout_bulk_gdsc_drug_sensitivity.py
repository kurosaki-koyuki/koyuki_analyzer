# -*- coding: utf-8 -*-
"""
bulk GDSC药物敏感性分析界面UI布局脚本 - 只负责创建控件、规划窗口布局、摆放按钮/输入框/画布、设置样式尺寸
完全不写按钮点击、触发逻辑
"""
...
class BulkGdscDrugSensitivityPageUI:
    def __init__(self, parent_widget, screen_width, screen_height):
        self.parent = parent_widget
        self.screen_width = screen_width
        self.screen_height = screen_height
        self.bulk_gdsc_drug_sensitivity_page = None
        self.create_page()

    def update_background(self):
        styles = get_mod_styles()
        paths = get_mod_paths()
        bg_label = self.bulk_gdsc_drug_sensitivity_page.findChild(QLabel, "bulk_gdsc_drug_sensitivity_bg")
        if bg_label:
    ...
    def update_styles(self):
        ...
        title_label = self.bulk_gdsc_drug_sensitivity_page.findChild(QLabel, "bulk_gdsc_drug_sensitivity_title")
        if title_label:
            title_label.setStyleSheet(f"color: {styles.get('sub_mutant_color', styles.get('mutant_color', '#E91E63'))};")

        overlay = self.bulk_gdsc_drug_sensitivity_page.findChild(QWidget, "bulk_gdsc_drug_sensitivity_overlay")
        if overlay:
    ...
    def create_page(self):
        self.bulk_gdsc_drug_sensitivity_page = QWidget(self.parent)

        styles = get_mod_styles()
        paths = get_mod_paths()
        mod_instance = global_mod_manager.get_current_mod()

        bg_label = QLabel(self.bulk_gdsc_drug_sensitivity_page)
        bg_label.setObjectName("bulk_gdsc_drug_sensitivity_bg")
        ...
        overlay = QWidget(self.bulk_gdsc_drug_sensitivity_page)
        overlay.setObjectName("bulk_gdsc_drug_sensitivity_overlay")
        ...
        self.btn_back_bulk_gdsc_drug_sensitivity = create_styled_button("← 返回上一页", font_size=12)
        top_layout.addWidget(self.btn_back_bulk_gdsc_drug_sensitivity)

        title_label = QLabel("GDSC药物敏感性分析")
        title_label.setObjectName("bulk_gdsc_drug_sensitivity_title")
        ...
        self.music_controller = MusicControllerClass(self.bulk_gdsc_drug_sensitivity_page, mod_instance)
        ...
        return self.bulk_gdsc_drug_sensitivity_page