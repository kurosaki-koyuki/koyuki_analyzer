...
class BulkKmPyPageUI:
    ...
    def create_page(self):
        ...
        self.bulk_km_page.setStyleSheet("background: transparent;")

        styles = get_mod_styles()
        mod_instance = global_mod_manager.get_current_mod()

        layout = QVBoxLayout(self.bulk_km_page)
        layout.setContentsMargins(20, 20, 20, 20)

        # 顶部标题栏
        top_layout = QHBoxLayout()

        self.btn_back_bulk_km = create_styled_button("← 返回上一页", font_size=12)
        top_layout.addWidget(self.btn_back_bulk_km)

        title_label = QLabel("bulk KM曲线 (Python版本)")
