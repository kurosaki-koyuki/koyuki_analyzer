...
class DiffAnalysis:
    ...
    def run_diff_analysis(self, group_col, selected_groups, method="mannwhitney", 
        ...
        Args:
            group_col: 分组列名
            selected_groups: 选中的分组列表，格式为 [[group1_items], [group2_items]]
                             支持多选组比较
            method: 检验方法 ('mannwhitney' 或 'scanpy_mast')
            ...
            filter_mask: 细胞筛选掩码（Series，True表示保留该细胞）
        ...
        if not selected_groups or len(selected_groups) != 2:
            raise ValueError("selected_groups 格式错误，应为 [[group1_items], [group2_items]]")
        
        group1_items = selected_groups[0]
        group2_items = selected_groups[1]
        
        if len(group1_items) == 0 or len(group2_items) == 0:
            raise ValueError("请至少选择1个组别1和1个组别2")
        ...
        # 与屎山代码完全一致的处理流程：
        # 1. 先应用细胞筛选掩码
        filtered_data = self.adata.copy()
        if filter_mask is not None:
            filtered_data = filtered_data[filter_mask.values]
        
        # 2. 从筛选后的数据中提取两组细胞的索引（与屎山代码一致）
        if group1_items:
            group1_cells = filtered_data.obs[filtered_data.obs[group_col].isin(group1_items)].index.tolist()
        else:
            group1_cells = []
        
        if group2_items:
            group2_cells = filtered_data.obs[filtered_data.obs[group_col].isin(group2_items)].index.tolist()
        else:
            group2_cells = []
        ...
        return self._run_mannwhitney_analysis(
            group_col, group1_items, group2_items, filtered_data,
            group1_cells, group2_cells,
            min_cells, min_expr, use_fdr, pval_threshold, logfc_threshold, pct_threshold
        )
    
    def _run_mannwhitney_analysis(self, group_col, group1_items, group2_items, filtered_data,
                                   group1_cells, group2_cells,
                                   min_cells, min_expr, use_fdr, pval_threshold, logfc_threshold, pct_threshold):
        """使用 Mann-Whitney U 检验进行差异分析（支持多选组）
        
        完全按照屎山代码逻辑实现，确保结果一致：
        1. 不进行基因预筛选
        2. 使用相同的CP10K标准化和log1p转换
        3. 使用相同的Mann-Whitney U检验参数
        4. 使用相同的FDR校正和排序方式
        """
        group1_name = "+".join(group1_items) if len(group1_items) > 1 else group1_items[0]
        group2_name = "+".join(group2_items) if len(group2_items) > 1 else group2_items[0]
        
        X = filtered_data.X
        if issparse(X):
            X = X.tocsr()
        
        all_cells = filtered_data.obs.index.tolist()
        ...
        # 构建结果DataFrame
        gene_names = filtered_data.var.index.tolist()
        ...
        # 排序（与屎山代码一致：p_val_adj_BH, p_val, gene）
        results = results.sort_values(['p_val_adj', 'p_val', 'gene']).reset_index(drop=True)
    ...
    def export_png(self, save_path):
        ...
        group1_name = "+".join(self.selected_groups[0]) if self.selected_groups and len(self.selected_groups) > 0 else "组1"
        group2_name = "+".join(self.selected_groups[1]) if self.selected_groups and len(self.selected_groups) > 1 else "组2"
        ax.set_title(f'Differential Expression: {group1_name} vs {group2_name}', fontsize=14)
