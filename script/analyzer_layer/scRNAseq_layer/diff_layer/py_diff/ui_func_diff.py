...
class DiffFunc(TableSearcherMixin):
    """差异分析前端功能类 - 纯前端显示操作"""
    ...
    def update_group_lists(self, group_col, unique_vals):
        """更新两个组别列表"""
        if hasattr(self.diff_ui, 'diff_group1_list'):
            self.fill_list_widget(self.diff_ui.diff_group1_list, unique_vals, select_all=False)
        if hasattr(self.diff_ui, 'diff_group2_list'):
            self.fill_list_widget(self.diff_ui.diff_group2_list, unique_vals, select_all=False)
    ...
    def update_diff_stats(self, df, group1_items, group2_items=None):
        ...
        Args:
            df: 差异分析结果DataFrame
            group1_items: 组别1的分组列表（可多选）
            group2_items: 组别2的分组列表（可多选）
        """
    ...
    def update_stats_info(self, df, df_up, df_down):
        """更新统计信息显示"""
        if not hasattr(self.diff_ui, 'diff_group1_cell_label'):
            return
        ...
        self.diff_ui.diff_group1_cell_label.setText(f"组1细胞数: {n_cells_group1}")
        self.diff_ui.diff_group2_cell_label.setText(f"组2细胞数: {n_cells_group2}")
        self.diff_ui.diff_up_label.setText(f"组1显著上调: {up_count}")
        self.diff_ui.diff_down_label.setText(f"组1显著下调: {down_count}")
        self.diff_ui.diff_stable_label.setText(f"稳定基因: {stable_count}")
        self.diff_ui.diff_total_label.setText(f"总基因: {total_count}")
    ...
    def _setup_gene_search(self):
        """初始化基因搜索功能"""
        if not hasattr(self.diff_ui, 'diff_result_tabs'):
            return
        tables = [
            getattr(self.diff_ui, 'diff_result_table', None),
            getattr(self.diff_ui, 'diff_result_table_up', None),
            getattr(self.diff_ui, 'diff_result_table_down', None),
        ]
        tables = [t for t in tables if t is not None]
        self.setup_gene_search(self.diff_ui.diff_result_tabs, tables, gene_col=0)
