...
class RDiffPageUI:
    ...
    def update_background(self):
        ...
        paths = get_mod_paths()
        bg_label = self.r_diff_page.findChild(QLabel, "r_diff_bg")
        if bg_label:
    ...
    def update_styles(self):
        ...
        title_label = self.r_diff_page.findChild(QLabel, "r_diff_title")
        if title_label:
        ...
        for child in self.r_diff_page.findChildren(QPushButton):
            if child.objectName() and child.objectName().startswith("number_input_btn_"):
                continue
            if child == self.btn_run_r_diff:
                continue
        ...
        self.btn_run_r_diff.setStyleSheet(get_stylesheet_for_widget('run_button'))
        ...
        """
        for child in self.r_diff_page.findChildren(QTabWidget):
            child.setStyleSheet(tab_style)
        
        if hasattr(self, 'r_diff_group1_cell_label'):
            self.r_diff_group1_cell_label.setStyleSheet(f"color: {styles.get('sub_mutant_color', '#98FB98')};")
        if hasattr(self, 'r_diff_group2_cell_label'):
            self.r_diff_group2_cell_label.setStyleSheet(f"color: {styles.get('sub_text_color', '#FFB6C1')};")
        if hasattr(self, 'r_diff_up_label'):
            self.r_diff_up_label.setStyleSheet(f"color: {styles.get('sub_mutant_color', '#FF6B35')};")
        if hasattr(self, 'r_diff_down_label'):
            self.r_diff_down_label.setStyleSheet(f"color: {styles.get('sub_text_color', '#87CEEB')};")
        if hasattr(self, 'r_diff_stable_label'):
            self.r_diff_stable_label.setStyleSheet(f"color: {styles.get('sub_border_color', '#666666')};")
        if hasattr(self, 'r_diff_total_label'):
            self.r_diff_total_label.setStyleSheet(f"color: {primary_color};")
        ...
        """
        for child in self.r_diff_page.findChildren(QWidget):
            if child.objectName() and child.objectName().startswith("styled_panel"):
                child.setStyleSheet(panel_style)
        
        overlay = self.r_diff_page.findChild(QWidget, "r_diff_overlay")
        if overlay:
            overlay.setStyleSheet(f"background: {styles.get('overlay_background', styles.get('sub_fill_color', 'rgba(26, 26, 46, 0.3)'))};")
    
    def create_page(self):
        self.r_diff_page = QWidget(self.parent)
        
        styles = get_mod_styles()
        paths = get_mod_paths()
        
        bg_label = QLabel(self.r_diff_page)
        bg_label.setObjectName("r_diff_bg")
        ...
        overlay = QWidget(self.r_diff_page)
        overlay.setObjectName("r_diff_overlay")
        ...
        title_label = QLabel("R版本差异基因分析")
        title_label.setObjectName("r_diff_title")
        ...
        self.music_controller = MusicControllerClass(self.r_diff_page, mod_instance)
        ...
        self.r_diff_group_combo = create_styled_combo_box()
        left_layout.addWidget(self.r_diff_group_combo)
        
        left_layout.addSpacing(5)
        
        group1_label = create_styled_label("组别1（多选）", font_size=10, bold=True)
        left_layout.addWidget(group1_label)
        self.r_diff_group1_list = create_styled_list_widget(fixed_height=80, multi_selection=True)
        left_layout.addWidget(self.r_diff_group1_list)
        
        group2_label = create_styled_label("组别2（多选）", font_size=10, bold=True)
        left_layout.addWidget(group2_label)
        self.r_diff_group2_list = create_styled_list_widget(fixed_height=80, multi_selection=True)
        left_layout.addWidget(self.r_diff_group2_list)
        ...
        self.r_diff_filter1_col = create_styled_combo_box()
        left_layout.addWidget(self.r_diff_filter1_col)
        self.r_diff_filter1_list = create_styled_list_widget(fixed_height=60, multi_selection=True)
        left_layout.addWidget(self.r_diff_filter1_list)
        
        filter2_col_label = create_styled_label("筛选条件2:", font_size=9, bold=True)
        left_layout.addWidget(filter2_col_label)
        self.r_diff_filter2_col = create_styled_combo_box()
        left_layout.addWidget(self.r_diff_filter2_col)
        self.r_diff_filter2_list = create_styled_list_widget(fixed_height=60, multi_selection=True)
        left_layout.addWidget(self.r_diff_filter2_list)
        ...
        self.r_diff_method_combo = create_styled_combo_box()
        self.r_diff_method_combo.addItems(["Seurat FindMarkers"])
        self.r_diff_method_combo.setCurrentIndex(0)
        self.r_diff_method_combo.setEnabled(False)
        self.r_diff_method_help_btn = create_questions_button(
            "使用Seurat包的FindMarkers函数进行差异表达分析。\n"
            "检验方法为MAST，基于负二项分布模型，适用于单细胞count矩阵。\n"
            "分析前需先在主页加载Seurat对象（.rds文件）。\n"
            "自动进行FDR校正。"
        )
        method_layout.addWidget(method_label)
        method_layout.addWidget(self.r_diff_method_combo)
        method_layout.addWidget(self.r_diff_method_help_btn)
        ...
        self.r_diff_min_cells = create_styled_spinbox(min_value=1, max_value=1000, default_value=3)
        min_cells_layout.addWidget(min_cells_label)
        min_cells_layout.addWidget(self.r_diff_min_cells)
        ...
        self.r_diff_min_expr = create_styled_spinbox(min_value=0, max_value=100, default_value=0)
        min_expr_layout.addWidget(min_expr_label)
        min_expr_layout.addWidget(self.r_diff_min_expr)
        left_layout.addLayout(min_expr_layout)
        
        pval_layout = QHBoxLayout()
        pval_label = create_styled_label("p值阈值", font_size=10, bold=False)
        self.r_diff_pval_spin = QDoubleSpinBox()
        self.r_diff_pval_spin.setFont(get_font_for_widget('label', 9))
        self.r_diff_pval_spin.setRange(0.001, 0.1)
        self.r_diff_pval_spin.setSingleStep(0.001)
        self.r_diff_pval_spin.setValue(0.05)
        self.r_diff_pval_spin.setStyleSheet(get_stylesheet_for_widget('spinbox'))
        pval_layout.addWidget(pval_label)
        pval_layout.addWidget(self.r_diff_pval_spin)
        ...
        self.r_diff_logfc_spin = QDoubleSpinBox()
        self.r_diff_logfc_spin.setFont(get_font_for_widget('label', 9))
        self.r_diff_logfc_spin.setRange(0, 2.0)
        self.r_diff_logfc_spin.setSingleStep(0.05)
        self.r_diff_logfc_spin.setValue(0.25)
        self.r_diff_logfc_spin.setStyleSheet(get_stylesheet_for_widget('spinbox'))
        logfc_layout.addWidget(logfc_label)
        logfc_layout.addWidget(self.r_diff_logfc_spin)
        left_layout.addLayout(logfc_layout)
        
        pct_layout = QHBoxLayout()
        pct_label = create_styled_label("表达量百分比阈值", font_size=10, bold=False)
        self.r_diff_pct_spin = QDoubleSpinBox()
        self.r_diff_pct_spin.setFont(get_font_for_widget('label', 9))
        self.r_diff_pct_spin.setRange(0.0, 1.0)
        self.r_diff_pct_spin.setSingleStep(0.05)
        self.r_diff_pct_spin.setValue(0.1)
        self.r_diff_pct_spin.setStyleSheet(get_stylesheet_for_widget('spinbox'))
        pct_layout.addWidget(pct_label)
        pct_layout.addWidget(self.r_diff_pct_spin)
        left_layout.addLayout(pct_layout)
        
        self.r_diff_log = QTextEdit()
        self.r_diff_log.setReadOnly(True)
        self.r_diff_log.setMaximumHeight(80)
        self.r_diff_log.setFont(get_font_for_widget('label', 10))
        self.r_diff_log.setStyleSheet(get_stylesheet_for_widget('text_edit'))
        left_layout.addWidget(self.r_diff_log)
        ...
        self.r_diff_group1_cell_label = QLabel("组1细胞数: 0")
        self.r_diff_group1_cell_label.setFont(QFont("幼圆", 10))
        self.r_diff_group1_cell_label.setStyleSheet(f"color: {styles.get('sub_mutant_color', '#98FB98')};")
        row1_layout.addWidget(self.r_diff_group1_cell_label)
        
        row1_layout.addStretch()
        
        self.r_diff_group2_cell_label = QLabel("组2细胞数: 0")
        self.r_diff_group2_cell_label.setFont(QFont("幼圆", 10))
        self.r_diff_group2_cell_label.setStyleSheet(f"color: {styles.get('sub_text_color', '#FFB6C1')};")
        row1_layout.addWidget(self.r_diff_group2_cell_label)
        stats_layout.addLayout(row1_layout)
        
        row2_layout = QHBoxLayout()
        self.r_diff_up_label = QLabel("组1显著上调: 0")
        self.r_diff_up_label.setFont(QFont("幼圆", 10))
        self.r_diff_up_label.setStyleSheet(f"color: {styles.get('sub_mutant_color', '#FF6B35')};")
        row2_layout.addWidget(self.r_diff_up_label)
        
        row2_layout.addStretch()
        
        self.r_diff_down_label = QLabel("组1显著下调: 0")
        self.r_diff_down_label.setFont(QFont("幼圆", 10))
        self.r_diff_down_label.setStyleSheet(f"color: {styles.get('sub_text_color', '#87CEEB')};")
        row2_layout.addWidget(self.r_diff_down_label)
        stats_layout.addLayout(row2_layout)
        
        row3_layout = QHBoxLayout()
        self.r_diff_stable_label = QLabel("稳定基因: 0")
        self.r_diff_stable_label.setFont(QFont("幼圆", 10))
        self.r_diff_stable_label.setStyleSheet(f"color: {styles.get('sub_border_color', '#666666')};")
        row3_layout.addWidget(self.r_diff_stable_label)
        
        row3_layout.addStretch()
        
        self.r_diff_total_label = QLabel("总基因: 0")
        self.r_diff_total_label.setFont(QFont("幼圆", 10))
        self.r_diff_total_label.setStyleSheet(f"color: {styles.get('sub_text_color', '#87CEEB')};")
        row3_layout.addWidget(self.r_diff_total_label)
        ...
        self.r_diff_result_tabs = create_styled_tab_widget()
        right_layout.addWidget(self.r_diff_result_tabs)
        
        self.r_diff_table_all_page, self.r_diff_table_all_layout = create_styled_tab_page(self.r_diff_result_tabs, "总体列表")
        self.r_diff_result_table = create_styled_table()
        self.r_diff_table_all_layout.addWidget(self.r_diff_result_table)
        
        self.r_diff_table_up_page, self.r_diff_table_up_layout = create_styled_tab_page(self.r_diff_result_tabs, "显著上调")
        self.r_diff_result_table_up = create_styled_table()
        self.r_diff_table_up_layout.addWidget(self.r_diff_result_table_up)
        
        self.r_diff_table_down_page, self.r_diff_table_down_layout = create_styled_tab_page(self.r_diff_result_tabs, "显著下调")
        self.r_diff_result_table_down = create_styled_table()
        self.r_diff_table_down_layout.addWidget(self.r_diff_result_table_down)
        
        self.r_diff_volcano_page, self.r_diff_volcano_layout = create_styled_tab_page(self.r_diff_result_tabs, "火山图")
        self.r_diff_volcano_label = ZoomableImageLabel()
        self.r_diff_volcano_label.setStyleSheet(f"border: 1px solid {styles.get('sub_border_color', '#1E3A5F')}; background: {styles.get('sub_fill_color', 'rgba(0,0,0,0.3)')};")
        self.r_diff_volcano_label.setAlignment(Qt.AlignCenter)
        self.r_diff_volcano_layout.addWidget(self.r_diff_volcano_label)
        ...
        self.btn_run_r_diff = create_styled_button("▶ 执行R差异分析", font_size=11, button_type='run')
        run_layout.addWidget(self.btn_run_r_diff)
        
        run_layout.addSpacing(8)
        
        self.r_diff_help_btn = create_questions_button(
            "点击运行后需要一段时间才可以出结果，\n"
            "这期间请耐心等待，千万别瞎点反复点运行按钮否则会翻大车。\n\n"
            "分析完成后会自动显示差异基因列表和火山图。"
        )
        run_layout.addWidget(self.r_diff_help_btn)
