# -*- coding: utf-8 -*-
"""
自定义气泡图前端功能脚本 - 只负责前端显示、控件内容更新、图片渲染等
...
class ScTargetgeneBubbleFunc:
    """自定义气泡图前端功能类 - 纯前端显示操作"""

    def __init__(self, targetgene_bubble_ui, parent_widget=None):
    ...
    def display_image(self, fig_path):
        ...
        label = self.targetgene_bubble_ui.targetgene_bubble_image_label
        if label:
    ...
    def log(self, message):
        """追加日志消息"""
        if hasattr(self.targetgene_bubble_ui, 'targetgene_bubble_log'):
            self.targetgene_bubble_ui.targetgene_bubble_log.append(message)
